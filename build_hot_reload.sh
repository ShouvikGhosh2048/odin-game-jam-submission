...
odin build game -extra-linker-flags:"$EXTRA_LINKER_FLAGS" -define:RAYLIB_SHARED=true -build-mode:dll -out:game_tmp$DLL_EXT -debug
...
if pgrep -f game_hot_reload.bin > /dev/null; then
    ...
    odin build main_hot_reload -out:game_hot_reload.bin -debug
fi
